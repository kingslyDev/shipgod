...
class RecentScansManager {
  ...
  createContainer() {
    ...
    if ($(this.containerId).length === 0) {
      ...
      $('#scanStatus')
        .parent()
        .after(`<div id="${this.containerId.replace('#', '')}" class="recent-scans-container" style="display: none;"></div>`);
    } else {
  ...
  renderProfessionalScans(data) {
    ...
    const scanItemsHtml = sortedScans
      .map((scan, index) => {
    ...
    const progressText = progressCounters.length > 0 ? `<small class="progress-text">${progressCounters.join(', ')}</small>` : '';
